function runApp(numba) {
  ...
  orderFile1 = readTextFile(`bings/${getFirstWord(realPhrase)}.txt`);
  orderFile2 = readTextFile(`bings/${getSecondWord(realPhrase)}.txt`);
  ...
  document.addEventListener("keydown", function (event) {
    if (event.key === "Enter") {
      const wordOne = document
        .getElementById("textbox")
        .value.trim()
        .toLowerCase();
      const wordTwo = document
  ...
  function test(wordOne, wordTwo) {
    ...
    if (number1 != 0 && number1 != -1) {
      ...
      document.getElementById("score1").innerHTML = wordOne + "  is correct!";
    } else {
      document.getElementById("score1").innerHTML =
    ...
    if (number2 != 0 && number2 != -1) {
      ...
      document.getElementById("score2").innerHTML = wordTwo + " is correct!";
    } else {
      document.getElementById("score2").innerHTML =
    ...
    if (number1 > 1000 || number1 == -1) {
      ...
      document.getElementById("semantic_score1").value =
        25 - 100 * (number1 / 25000);
    } else if (number1 < 100 && number1 > 50) {
      document.getElementById("semantic_score1").value =
        50 - 100 * (number1 / 25000);
      if (long_johnson == 0) {
        long_johnson = counter + 1;
      }
    } else if (number1 < 50 && number1 > 1) {
      document.getElementById("semantic_score1").value =
        100 - 100 * (number1 / 25000) - 1;
      if (long_johnson == 0) {
        long_johnson = counter + 1;
      }
    } else if (number1 == 0) {
    ...
    if (number2 > 1000 || number2 == -1) {
      ...
      document.getElementById("semantic_score2").value =
        25 - 100 * (number2 / 25000);
    } else if (number2 < 100 && number2 > 50) {
      document.getElementById("semantic_score2").value =
        50 - 100 * (number2 / 25000);
      if (long_johnson < 0) {
        long_johnson = counter;
      }
    } else if (number2 < 50 && number2 > 1) {
      document.getElementById("semantic_score2").value =
        100 - 100 * (number2 / 25000) - 1;
      if (long_johnson < 0) {
        long_johnson = counter;
      }
    } else if (number2 == 0) {
    ...
    verbal_hint1 = generateVerbalHint(wordOne, getFirstWord(realPhrase));
    verbal_hint2 = generateVerbalHint2(wordTwo, getSecondWord(realPhrase));
  ...
  function generateVerbalHint(str1, str2) {
    for (let i = 0; i < str1.length; i++) {
      if (
        str2.includes(str1.charAt(i)) &&
        !correct1.substring(16).includes(str1.charAt(i))
      ) {
  ...
  function generateVerbalHint2(str1, str2) {
    for (let i = 0; i < str1.length; i++) {
      if (
        str2.includes(str1.charAt(i)) &&
        !correct2.substring(16).includes(str1.charAt(i))
      ) {
  ...
  function toggleshare() {
    ...
    document.getElementById("final-1").innerHTML =
      "The real phrase was: " + realPhrase;
    document.getElementById("final-2").innerHTML = "Backwords 0" + numba;
    let attemptEmojis = "";
    let yellow_number = 0;
    ...
    document.getElementById("final-3").innerHTML =
      counter + 1 + " Attempts " + attemptEmojis;
  ...
  function copyFunction() {
    ...
    let copyString = `Backwords 0${numba}
...
backwords.xyz`;
    ...
    let twitterString = `Backwords 0${numba}%0A
...
backwords.xyz%0A`;
  ...
  hint.addEventListener("click", function () {
    if (number <= 3) {
      let [hintFirstWord, hintSecondWord] = outputPhrase(
        realPhrase,
        number
      ).split(" ");
      ...
      if (!gotSecond && !gotFirst) {
        sharing.textContent =
          "the phrase is " + outputPhrase(realPhrase, number);
        number++;
      } else if (!gotSecond) {
        sharing.textContent =
  ...
  function getScore(userGuess, realPhrase, number) {
    ...
    if (userFirstWord === realFirstWord && userSecondWord === realSecondWord) {
      gotFirst = true;
      gotSecond = true;
      response = generateText(realPhrase, numba, counter);
